...
def run():
    ...
    valid_dataset = dataset.coco_bc(
        config,
        config.DATASET.ROOT,
        config.DATASET.TEST_SET, is_train, None)
    ...
    stride_x = config.MODEL.IMAGE_SIZE[0]/config.MODEL.EXTRA.HEATMAP_SIZE[0]
    stride_y = config.MODEL.IMAGE_SIZE[1]/config.MODEL.EXTRA.HEATMAP_SIZE[1]
    show_viz=True
    ...
    for i, (input, joints, fields, bc, joints_weight, meta) in enumerate(valid_loader):
        # logger.info("%d frame. Meta: %d \n %s", i, len(meta), meta)
        ...
        bc = bc.cpu().numpy()
        print("Shapes ",joints.shape, fields.shape, bc.shape)

        batch_predictions = get_batch_predictions(config, joints, fields, bc, c, s, batch_images)
        all_preds.extend(batch_predictions)
      
        # for the viz use only the first image
        if show_viz:
            if not is_train:
                full_img = cv2.imread(batch_images[0])
                sk_viz = visualize_skeletons(full_img, batch_predictions[:meta['num_pred'][0]])
                print("Skeletons found: ",len(batch_predictions)," original canvas ", full_img.shape)
                cv2.imshow("SKel Viz",sk_viz)

            msg = 'Test: [{0}/{1}] Preds {2}'.format( i, len(valid_loader),len(all_preds))
            logger.info(msg)

            
            hm = joints[0].transpose(1,2,0)
            fields = fields[0].transpose(1,2,0)
            bc = bc[0]
            
            hm_sum = np.sum(hm,axis=-1)
            hm_res = cv2.resize(hm_sum, (0,0),fx=stride_x,fy=stride_y)
            ...
            cv2.imshow("Fields RGB",cv2.resize(fields_rgb, (0,0),fx=stride_x,fy=stride_y))
        ...
        if k&0xFF==ord('p'):
            paused = not paused
    print("Running validation on {0} predictions ".format(len(all_preds)))
    name_values, perf_indicator = valid_dataset.evaluate( config, all_preds, "tmp")

    print("PERF: ",perf_indicator)
